...
public class ArrayQueue<E> extends AbstractQueue<E>
	...
	public boolean hasNext () {
	    // 
		// if the index is more than the last index
		// that means there is nothing left to access
		if (index <= last){
			return true;
		}
	...
	///
		...
		Object helper;
		Iterator cusIter = this.iterator();


		while (cusIter.hasNext()){ //while cusIter still has content
			helper = cusIter.next();	//store the customer in helper

			newElements[n] = (E) (helper); // add the stored element into the new array
			n++; //increase n index by 1
		}

	///

	first = 0;
	last = n - 1; //update last so it accounts for the new size
	theElements = newElements; //replace theElements with newElements
