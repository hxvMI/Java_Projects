...
public class Heap <E> extends AbstractQueue<E> {
    ...
    */
	...
	///
		...
		int lastElement = size() - 1; //index of last element
		theElements.set(0,theElements.remove(lastElement));
		//call remove to remove element then put its output at 1
	...
	///
		...
		while((isValid(left(index)) && compare(index,left(index)) > 0) ||
			...
			boolean hasRight = isValid(rightIndex);
			boolean hasLeft = isValid(leftIndex);
			...
			///
			if (hasRight && compare(rightIndex,leftIndex) < 0){
					// Swap with right child (and set index).
					///
					index = swap(index, rightIndex);
//				}
				// Else swap with left child.
				///
			}
			else index = swap(index, leftIndex);
		}
	...
	///
		...
		//index will be -1 if it isn't in the list
		//return false cuz not removed
	...
	///
		...
		if (index == size()-1){//last item is at size-1
			theElements.remove(index);
			return true; //return true cuz removed
		}
	...
	///
		...
		E lastElem = theElements.remove(size()-1);//remove last element
		theElements.set(index,lastElem);//replace element at index
	...
	///
		...
		//and while index > parent(index) compare will a return positive
		while (compare(index, parent(index)) < 0){
			index = swap(index, parent(index));
		}
	...
	///
		...
		while ((isValid(left(index)) && compare(index,left(index)) > 0) //while index is greater than either index
		|| (isValid(right(index)) && compare(index,right(index)) > 0)){

			boolean validRight = isValid(right(index)); //store valid right
			boolean validLeft = isValid(left(index)); //store valid left

			//if right is valid and right > left swap index with right
			if (validRight && compare(right(index),left(index)) < 0) index = swap(index,right(index));
				else index = swap(index,left(index)); //else this means left is bigger so swap with bigger
