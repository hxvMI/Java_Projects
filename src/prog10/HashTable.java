...
//	 System.out.println("put " + key + " " + value + " hash index " + hashIndex(key));
	...
	if (entry != null) {
	    ...
	    V temp = entry.getValue(); //store old value
...
//	for (Entry currEntry = table[index]; currEntry != null; currEntry = currEntry.next){
...
//	}
	...
	while (currEntry != null){
		if (currEntry.getKey().equals(key)
				|| key.equals(currEntry.key)
				|| currEntry.key.equals(key)){
			return currEntry;
		}
...
//	    return;
	...
	for (int i = 0; i < oldTable.length; i++) {
		...
		Entry currEntry = oldTable[i];
		while(currEntry != null){
			int newIndex = hashIndex(currEntry.key);
...
//	table = oldTable;
	...
	Entry currentEle = table[index];

	if (currentEle.getKey().equals(key)){ //if equal set tabl[index] to curr.next and decrease size
		table[index] = currentEle.next;
		size--;
		return currentEle.getValue(); //then return the value
	}
	...
	Entry previousEle = null;

	while (currentEle != null){ //while not null check keys
		if (currentEle.getKey().equals(key)){ //if true prev.next to curr.next
			previousEle.next = currentEle.next;
			size--;
			return currentEle.getValue();
		}
		previousEle = currentEle; //move up values
	...
	private EntryIterator () {
		...
		for (index = 0; index < table.length; index++) {
			if (table[index] != null){ // if not null set to entry and break
				entry = table[index];
				break;
			}
		}
	...
	public boolean hasNext () {
	    ...
	    return entry != null; //entry is the value of table[index]
	...
	public Map.Entry<K, V> next () {
		...
		while (entry == null && index < table.length){ // if null continue till end
			index++; //have to put in at top, if put at bottom causes duplicates
