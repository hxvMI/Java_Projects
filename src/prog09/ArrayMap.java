...
public class ArrayMap <K extends Comparable<K>, V>
	...
	while (low <= high){
		int mid = (low+high)/2;
		int compare = key.compareTo(entries[mid].key); //compare the key to the entry at the middles key

		if (compare < 0)high = mid - 1;
		else if (compare > 0)low = mid + 1;
		else return mid; //if keys are equal return the middle index
	}
	...
	///
		...
		//start from the back and shift all entries till the index
		//to the right
		for (int i = size; i > index; i--) {
			entries[i] = entries[i-1];
		}

		//insert newEntry at index
	...
	///
		...
		//from index till end set curr index to next index
		for (int i = index; i < size - 1; i++) {
			entries[i] = entries[i+1];
		}

		//set last entry to null and decrease size
		entries[size-1] = null;
		size--;
