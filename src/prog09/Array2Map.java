...
public class Array2Map <K extends Comparable<K>> extends ArrayMap<K, Object> {
	...
	///
		...
		//if index is valid we getMap using the index
		//then if map.contains the key
		// return map.get(key)
		K key = (K) keyAsObject;
		int index = findMap(key);
		if (index >= 0){
	...
	///
		...
		//if less than min set index to current min
		if (index == -1){
			index = 0;
		}//if more than max set index to current max
	...
	///
		...
		// conditionals
		if (index == -1) index = 0;
		else if (-index -1 == size) index = size -1;
		...
		if (map.size == 1) {
				if (index < size-1){
					...
					// split it
					if (map.size > 3) split(index); //split if bigger than 3
				}
				...
				// How do we call join?
					...
					ArrayMap<K, Object> previousMap = getMap(index-1); //reduce size


					// If the previousmap at index is too large
					// split it
					if (previousMap.size > 3) split(index-1); //split if bigger than 3
				}
			}
