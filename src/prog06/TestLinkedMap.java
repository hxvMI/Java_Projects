...
public class TestLinkedMap extends LinkedMap {
	...
	boolean testRemove (Entry[] in, Entry newEntry, Entry[] out) {
//		newEntry.previous = null;
//		newEntry.next = null;
		set(in);
		try {
			if (newEntry != null && found(newEntry, newEntry.key)){
				remove(newEntry);
			}
			if (out.length == 0) {
				if (size() != 0) {
	...
	void testRemove() {
		...
		{
			Entry[] in = {a1, b2, c3};
			Entry[] out = {a1, b2, c3}; // removing an entry that does not exist
			Entry nonExistent = new Entry("x", "0");
			check(testRemove(in, nonExistent, out));
		}
		{
			Entry[] in = {a1, b2, c3};
			Entry[] out = {a1, b2, c3}; // removig a null entry
			check(testRemove(in, null, out));
		}
	}
